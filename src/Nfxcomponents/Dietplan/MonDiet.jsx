...
const Monday = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/week`)
      .then((res) => res.json())
      .then((data) => {
        const mondayData = data.find(
          (dayPlan) => dayPlan.day.toLowerCase() === "monday"
        );
        setDayData(mondayData);
        setLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <section id="diet-plan" className="diet-plan-section">
      <div className="container">
        <button id="bckbmi" onClick={() => navigate("/exercises/chest-triceps")}>
          Back
        </button>
        <h2 className="section-title">Monday Diet Plan & Exercise Guide</h2>
