...
const Thursday = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/week`)
      .then((res) => res.json())
      .then((data) => {
        const thursData = data.find(
          (dayPlan) => dayPlan.day.toLowerCase() === "thursday"
        );
        setDayData(thursData);
        setLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <section id="diet-plan" className="diet-plan-section">
      <div className="container">
        <button id="bckbmi" onClick={() => navigate("/exercises/shoulders")}>
          Back
        </button>
        <h2 className="section-title">Thrusday Diet Plan</h2>
