...
const Wednesday = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/week`)
      .then((res) => res.json())
      .then((data) => {
        const wedData = data.find(
          (dayPlan) => dayPlan.day.toLowerCase() === "wednesday"
        );
        setDayData(wedData);
        setLoading(false);
      })
  ...
  return (
    <section id="diet-plan" className="diet-plan-section">
      <div className="container">
        <button id="bckbmi" onClick={() => navigate("/exercises/legs")}>
          Back
        </button>
        <h2 className="section-title">Wednesday Diet Plan</h2>
