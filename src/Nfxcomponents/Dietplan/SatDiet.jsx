...
const Saturday = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/week`)
      .then((res) => res.json())
      .then((data) => {
        const satdayData = data.find(
          (dayPlan) => dayPlan.day.toLowerCase() === "saturday"
        );
        setDayData(satdayData);
        setLoading(false);
      })
      .catch((err) => {
  ...
  return (
    <section id="diet-plan" className="diet-plan-section">
      <div className="container">
        <button id="bckbmi" onClick={() => navigate("/exercises/Flexible")}>
          Back
        </button>
        <h2 className="section-title">Saturday Diet Plan</h2>
