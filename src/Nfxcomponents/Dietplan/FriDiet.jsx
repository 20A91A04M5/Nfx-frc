...
const Friday = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/week`)
      .then((res) => res.json())
      .then((data) => {
        const friData = data.find(
          (dayPlan) => dayPlan.day.toLowerCase() === "friday"
        );
        setDayData(friData);
        setLoading(false);
      })
  ...
  return (
    <section id="diet-plan" className="diet-plan-section">
      <div className="container">
        <button id="bckbmi" onClick={() => navigate("/exercises/Cardio")}>
          Back
        </button>
        <h2 className="section-title">Friday Diet Plan</h2>
